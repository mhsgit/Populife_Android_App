...
public class HexUtil {
	 ...
	 * 10进制数字 转成 16进制字符串
	 *
	 * @param number     转换的 10 进制对象
	 * @param byteLength 返回结果的字节长度（2 个 16 进制字符为 1 个字节长度，如：0001 为 4 个字符，2 个字节）
	 */
	public static String int2HexStr(int number, int byteLength) {
		int len = byteLength * 2; // 计算字符长度
		String format = "%0" + len + "X";
	 ...
	 * @param byteLength 返回结果的字节长度（2 个 16 进制字符为 1 个字节长度，如：0001 为 4 个字符，2 个字节）
	 */
	public static String intLetterHexStr(int number, int byteLength) {
		int len = byteLength; // 计算字符长度
		String format = "%0" + len + "X";
		return String.format(format, number);
	}
	 ...
	 * @param str 10进制字符串
	 */
	public static String intStr2HexStr(String str) {
		StringBuilder sb = new StringBuilder();
		for (int i = 0; i < str.length(); i++) {
			sb.append(String.format("0%s", str.substring(i, i + 1)));
		}
	...
	public static String hexStr2Ascii(String hexStr) {
		...
		for (int i = 0; i < hexStr.length(); i += 2) {
			sb.append((char) Integer.parseInt(hexStr.substring(i, i + 2), 16));
		}
		return sb.toString();
	}
	 ...
	 * ASCII 码 ---> 十六进制的字符串
	 *
	 * @param asciiStr ASCII 类型的字符串
	 * @return 十六进制的字符串
	 */
	public static String ascii2HexStr(String asciiStr) {
