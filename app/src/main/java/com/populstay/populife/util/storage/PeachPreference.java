...
public class PeachPreference {
	...
	private static final String LAST_UNLOCK_TIME = "last_unlock_time"; //记录最近一次解锁时间
	private static final String LAST_UNLOCK_TYPE = "last_unlock_type"; //记录最近一次解锁类型
	private static final String SHOW_LOCK_ADMIN_CODE = "lock_show_admin_code"; //管理员密码是否展示密码列表中
	private static final String ADMIN_CODE_NAME = "admin_code_name"; //管理员密码名称(本地存储)
	public static final String KEY_LOCK_ACTION_TYPE = "key_lock_action_type"; // 锁操作类型
	 ...
	 * 是否支持生物识别
	 */
	public static boolean isSupportBiometic() {
		return getAppPreference()
				.getBoolean(VAL_SUPPORT_BIOMETRC + readUserId(), false);
	}
	 ...
	 * 设置是否使用指纹验证登录
	 *
	 * @param supportBiometrc 是否使用 supportBiometrc
	 */
	public static void setValSupportBiometrc(boolean supportBiometrc) {
		getAppPreference()
				.edit()
				.putBoolean(VAL_SUPPORT_BIOMETRC + readUserId(), supportBiometrc)
				.apply();
	}
	...
	// type：2 为远程，1为蓝牙
	public static void setLastUnlockTimeAndType(int lockId, long time, int type) {
		getAppPreference()
				.edit()
				.putLong(LAST_UNLOCK_TIME + readUserId() + lockId, time)
				.putInt(LAST_UNLOCK_TYPE + readUserId() + lockId, type)
				.apply();
	}

	public static void showLockAdminCode(int lockId, boolean isShow) {
		getAppPreference()
				.edit()
				.putBoolean(SHOW_LOCK_ADMIN_CODE + readUserId() + lockId, isShow)
				.apply();
	}

	public static boolean isShowLockAdminCode(int lockId) {
		return getAppPreference()
				.getBoolean(SHOW_LOCK_ADMIN_CODE + readUserId() + lockId, true);
	}

	public static String getAdminCodeName(int lockId) {
		return getAppPreference()
				.getString(ADMIN_CODE_NAME + readUserId() + lockId, "");
	}

	public static void saveAdminCodeName(int lockId, String adminCodeName) {
		getAppPreference().edit().putString(ADMIN_CODE_NAME + readUserId() + lockId, adminCodeName).apply();
	}

	public static long getLastUnlockTime(int lockId) {
		return getAppPreference()
				.getLong(LAST_UNLOCK_TIME + readUserId() + lockId, 0);
	}
