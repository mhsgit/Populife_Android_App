...
//		String str = "/^.*(?=.{6,})(?=.*\\d)(?=.*[A-Za-z]).*$/";
	...
	public static boolean isEmail(String email) {
		...
		// 前面是优化逻辑，做初步判断，毕竟复杂的正则比较耗时（邮箱尾缀支持添加@后面 . 10个字符）
		String strPattern = "^([a-zA-Z0-9_\\.-]+)@([\\da-z\\.-]+)\\.([a-z\\.]{2,10})$";
	...
	public static boolean isMobileNum(String mobiles) {
		...
		return m.matches();*/
		return isPhoneNumberValid(mobiles);
	}
	...
	public static boolean isPhoneNumberValid(String phoneNumber, String countryCode) {
		...
		if (countryCode.contains("+") && countryCode.length() > 1){
			countryCode = countryCode.substring(countryCode.indexOf(("+"))+1);
		}
		PhoneNumberUtil phoneUtil = PhoneNumberUtil.createInstance(MyApplication.getApplication());
		try {
			// "+8615820799999","86"
