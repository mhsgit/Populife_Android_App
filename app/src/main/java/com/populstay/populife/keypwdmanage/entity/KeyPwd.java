...
public class KeyPwd implements Parcelable {
	...
	// String钥匙的状态（110401：正常使用，110402：待接收，110405：已冻结，110408：已删除，110410：已重置,110500:已过期）;;;
	// 密码状态(0删除，1未激活，2失效， 3正常);;;门卡状态（1：有效，2：待生效，3：失效）
	private String status;
	// Integer 1限时钥匙;;;指纹/门卡类型（1限时，2永久）
	private int type;
	//String用户头像
	...
	// Integer键盘密码类型（15为自定义密码,其他参考科技侠平台）(-1为管理员密码，Android App端自己添加的用于逻辑控制的，与后台无关)
	...
	// 指纹 id（手动转换键名 id 为 fpStringId，避免和 int 类型的 id 冲突）
	...
	protected KeyPwd(Parcel in) {
		...
		fpStringId = in.readString();
		fingerprintId = in.readString();
		fingerprintNumber = in.readString();
	...
	public void setUseStaus(int useStaus) {
	...
	public void setFpStringId(String fpStringId) {
		this.fpStringId = fpStringId;
	}
	...
	public void writeToParcel(Parcel dest, int flags) {
		...
		dest.writeInt(useStaus);
		dest.writeInt(cardId);
		dest.writeString(cardNumber);
		dest.writeString(fpStringId);
		dest.writeString(fingerprintId);
		dest.writeString(fingerprintNumber);
